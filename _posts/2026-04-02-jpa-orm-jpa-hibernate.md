...
categories: [JPA, ORM / JPA / Hibernate 이론]
...
```java
// memberDAO.findById()가 실행하는 SQL:
// SELECT * FROM member WHERE id = ?
// 이 SQL은 team 정보를 JOIN하지 않기 때문에 아래 코드는 NPE가 발생할 수 있다
...
JDBC는 조회할 때마다 `new Member()`로 새 인스턴스를 만들기 때문에 같은 PK를 가진 두 객체라도 `==` 비교가 `false`다. 컬렉션에서는 자연스럽게 동일성이 보장되지만, DB에서 가져온 객체는 그렇지 않다.
...
프록시의 상세 동작은 Phase 6 FetchType에서 깊이 다룬다.
...
`SessionFactory`는 애플리케이션 당 하나만 생성한다. Spring Boot는 이를 자동으로 관리한다. `Session`은 하나의 요청(트랜잭션)마다 생성되고 종료되는 가벼운 객체다. 영속성 컨텍스트를 내부에 가지고 있으며, 이 안에서 1차 캐시, 더티 체킹, 쓰기 지연이 동작한다. 영속성 컨텍스트의 상세 동작은 Phase 2에서 핵심으로 다룬다.
